...
# ==========================================
# 3. COMPONENTI TRAINER E COLLATOR V3
# ==========================================
...
class STLEncKernelTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        ...
        # Similarità latente (Modello)
        S_model = torch.matmul(z, z.T)
        logits = S_model / TEMPERATURE  
        
        # 1. Hard Pair Reweighting (Focus sull'errore semantico locale)
        # Rende più forti i gradienti dove il modello e il kernel discordano maggiormente
        with torch.no_grad():
            error_weight = torch.abs(K_target - S_model.detach())
            # error_weight = torch.pow(error_weight, 2) 
            error_weight = torch.clamp(error_weight / (error_weight.mean() + 1e-8), max = 3.0)
        
        logits_max, _ = torch.max(logits, dim=1, keepdim=True)
        exp_logits = torch.exp(logits - logits_max.detach())
        
        # Applichiamo i pesi degli errori alla distribuzione Soft-InfoNCE
        weighted_exp = exp_logits * error_weight
        
        numerator = torch.sum(weighted_exp * K_target, dim=1)
        denominator = torch.sum(weighted_exp, dim=1)
        
        loss = -torch.log(numerator / (denominator + 1e-8)).mean()
        
        if model.training and self.state.global_step % self.args.logging_steps == 0:
            with torch.no_grad():
                # Metriche di Alignment e Uniformity
                ...
                wandb.log({
                    ...
                    "train/kernel_alignment": alignment_val.item(),
                    "train/uniformity": uniformity.item(),
                    "train/avg_latent_sim": S_model.mean().item(),
                    "train/max_error_weight": error_weight.max().item()
...
if __name__ == "__main__":
    wandb.init(project="stlenc-distillation", name="train-v3-fixed-kernel-hard-mining")
