...
TEMPERATURE = 0.3  # Ridotta per una separazione semantica più netta

# ==========================================
# 1. CODICE LOGICO STL (ARCHITETTURA NODI)
# ==========================================
# [Il codice dei nodi rimane invariato rispetto al tuo originale]
...
# 2. PARSER E MISURA
# ==========================================
# [Il codice Measure e Parser rimane invariato]
...
# ==========================================
# 3. COMPONENTI TRAINER E COLLATOR (MODIFICATI)
# ==========================================
...
class STLEncKernelTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        ...
        with torch.no_grad():
            ...
            # K_target è [B, B] con valori di similarità semantica in [0, 1]
        ...
        # LogSumExp per stabilità numerica
        logits_max, _ = torch.max(logits, dim=1, keepdim=True)
        exp_logits = torch.exp(logits - logits_max.detach())
        
        # --- NUOVA LOSS ---
        # Numeratore: pesiamo ogni exp(logit) per il valore reale nel kernel STL.
        # Include la diagonale (K_ii=1) e i vicini semantici (K_ij > 0).
        numerator = torch.sum(exp_logits * K_target, dim=1)
        
        # Denominatore: somma di tutte le exp-similarità nel batch (contrastive).
        ...
        if model.training and self.state.global_step % self.args.logging_steps == 0:
            with torch.no_grad():
                latent_sim = torch.matmul(z, z.T)
                # Monitoriamo quanto lo spazio latente riflette il kernel
                alignment = F.cosine_similarity(K_target.view(-1), latent_sim.view(-1), dim=0)
                wandb.log({
                    "train/loss": loss.item(),
                    "train/kernel_alignment": alignment.item(),
                    "train/avg_kernel_sim": K_target.mean().item(),
                    "train/avg_latent_sim": latent_sim.mean().item()
...
if __name__ == "__main__":
    # Usa un nuovo nome per distinguere l'esperimento distributivo
    wandb.init(project="stlenc-distillation", name="soft-infonce-alignment")

    model_id, new_repo = "saracandu/stlenc-distilled-v2", "saracandu/stlenc-distilled-v2"
    ...
    # Selezione per velocizzare la valutazione
